...
$$ L:W\xrightarrow{p} \mathcal{F} \xrightarrow{L*{\mathbb{F}}} \mathbb{R}$$
where  $\mathcal{F}=\\\{ f_w:\mathbb{R}^{|X|} \to \mathbb{R}^{|Y|} \mid w\in W \\\}$ is the function space of all possible $f_w$.
We stress that the parameter-function map is completely \_task-independent*! It does not take into account the training distribution.
