...
*By [YOUR NAME] · [DATE] · ~18 min read · Nothing here is investment advice or legal advice.*
...
I am going to make that general case first, because it applies to everything. Then I am going to spend most of the essay on the hardest, highest-stakes instance of it, private-credit covenant analysis, because if you can see why validation is non-negotiable *there*, you can see why it matters everywhere.
...
There is hard evidence that validation gets *harder* exactly where the work is most valuable. A 2026 framework for assessing intrinsic tabular hallucination in finance, built to measure how often a model's output contradicts the financial table it was given, found that hallucination errors rise as the information-extraction task grows more complex.[^faith] In other words, the more reasoning a financial task requires, summarize *and* calculate *and* interpret across a structured input, the more the model's unverified output drifts from the source. The implication is uncomfortable and important: the workflows where AI promises the most leverage, the multi-step, reasoning-heavy ones, are precisely the workflows where the validation layer matters most, because that is where the model is most likely to be confidently wrong.
...
`[FIGURE 1 — "A covenant is not a number — it's a chain." A nested/dependency diagram. Top: the maintenance covenant ("Leverage ≤ 5.0x — the easy part"). Below it, the chain that produces the number actually being tested: Leverage Ratio → depends on → Consolidated Total Debt (inclusions/exclusions) ÷ Consolidated EBITDA → which incorporates → permitted add-backs (capped · lookback-limited) + baskets/carve-outs + amendments/side letters. Annotate: "The threshold is trivial. The computed value is the output of a definition chain — and that's where it goes wrong." Caption: keep it sober and structural.]`
...
`[FIGURE 2 — "Extraction is table stakes. Validation is the moat." Two stacked bands. Top band, EXTRACTION (lighter/neutral): identify covenants · capture defined terms & dependencies · record thresholds, baskets, add-backs, step-downs, amendments → label "hard, and where most tools concentrate." Bottom band, VALIDATION (teal accent): structure matches the agreement clause-for-clause · computed ratios footed & reconciled · add-backs within caps/lookbacks · clause-level citation + confidence · abstain → human review for ambiguity → label "harder, less glamorous, and where defensibility lives." Caption: "A tool that extracts fluently and validates weakly is the 'looks finished, might be wrong' trap — applied to a number where wrong is a loss."]`

There is a useful way to think about how such a system should be built, drawn from the broader pattern in production financial AI. The more credible designs do not lean on a single model asked to do everything; they decompose the work across specialized agents with bounded roles and route the output through governance before it lands. The former quant Ben Lorica describes the emerging shape as crews of role-specific agents, one to extract, one to compute, one acting as a risk officer or auditor, coordinating through standard tool-use protocols rather than one monolithic prompt.[^multiagent] For covenant work the mapping is natural: an extraction agent pulls the defined-term structure, a calculation agent recomputes the ratios against reporting, and a checking agent reconciles the result and flags ambiguity for a human, with each step logged. The architecture is not the point in itself. The point is that decomposition plus governance is what lets you put a clause-level citation and a recomputed number in front of a credit committee and defend both.
...
`[FIGURE 3 — The Evidence Chain (reuse R1, the recurring brand figure), labelled for credit. Claim (covenant level / computed ratio) → Source (agreement § · page · defined term · amendment) → Calculation (recompute ratio · apply add-backs within caps · reconcile to reporting) → Reviewer (credit professional sign-off), with the "ambiguous / conflicting → human review" loop. Caption: "The same evidence chain as the rest of the series — pointed at the hardest documents in finance. This is the IC evidence pack."]`
...
This is Essay 06 of *The AI Operating Manual for Investment Firms*. It connects directly to the deterministic-extraction argument of the previous essay (why retrieval is not evidence) and to the broader point that an AI system gives you a distribution of answers, not the answer, so the validation layer is what makes any of it usable. The spine holds: the advantage is in the workflow, the evidence, and the controls, not the subscription, not the citation, and not the demo.
...
[^multiagent]: On multi-agent / "crew" architectures, namely decomposing a workflow across role-specific agents (e.g., extractor, calculator, risk officer, auditor) that coordinate through standard tool-use protocols such as the Model Context Protocol, rather than a single monolithic prompt, see Ben Lorica, "What's Emerging in Financial AI: From Foundation Models to Compliance-as-Code," Gradient Flow, January 13, 2026. The mapping to covenant work (extract → compute → reconcile/flag, each logged) is the author's application of that general pattern. https://gradientflow.substack.com/p/emerging-ai-patterns-in-finance-what

[^faith]: "FAITH: A Framework for Assessing Intrinsic Tabular Hallucinations in Finance," arXiv:2508.05201 (2025). Focuses on intrinsic hallucination, where a model's output is inconsistent with the financial table it was given, and finds that increasing information-extraction complexity (extract plus calculate plus interpret over a structured input) exacerbates hallucination errors, with the risk rising further when models rely on internal knowledge or web search rather than the provided financial input. https://arxiv.org/abs/2508.05201

[^practitioner]: A practitioner account of running leading models head-to-head on a real credit agreement (Stephen Smith, "Two AIs. One Credit Agreement.," 2026) observes that both models will hallucinate a defined-term cross-reference unless given the actual definitions section, producing output that is "wrong in ways that look right," with the EBITDA bridge identified as where errors hide. https://www.smithstephen.com/p/two-ais-one-credit-agreement-both (practitioner source; illustrative, and consistent with the FAITH findings above).
